...
def generate(state: State, budget_totals: dict, events_path: str | Path, out_path: str | Path='reports/report.md') -> str:
    events = []
    ep = Path(events_path)
    if ep.exists():
    ...
    lines = ['# Hehua 评测报告', '', '## 总览', f'- 题目数: {len(states)}\u3000已解: {len(solved)}\u3000部分: {len(partial)}', f'- flag 发现率: {flags}/{flag_total} = {flags / flag_total:.1%}', f'- 提交精度(1-误报率代理): {precision:.1%} ({len(correct_submits)}/{len(submits)})', f'- 总解题耗时: {elapsed_total / 60:.1f} min\u3000单题均时: {elapsed_total / n / 60:.1f} min', f'- LLM 调用次数: {len(llm_calls)}', f"- Token 消耗: {budget_totals.get('total_tokens', 0):,} (prompt {budget_totals.get('prompt_tokens', 0):,} / completion {budget_totals.get('completion_tokens', 0):,})", '', '## 按难度']
    ...
    for s in states:
        lines.append(f'| {s.code} | {s.status} | {s.flags}/{s.flag_count} | {s.attempts} | {s.elapsed:.0f} |')
    out = Path(out_path)
