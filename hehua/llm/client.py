...
class LLMClient:
    ...
    def chat(self, messages: list[dict], tools: list[dict] | None=None, model_role: str='primary', temperature: float=0.3, thinking: str | None=None) -> ChatResult:
        ...
        for _ in range(len(self.providers)):
            ...
            try:
                return self._call(provider, messages, tools, model_role, temperature, thinking)
    ...
    def _call(self, provider: Provider, messages: list[dict], tools: list[dict] | None, model_role: str, temperature: float, thinking: str | None=None) -> ChatResult:
        url = provider.base_url.rstrip('/') + '/chat/completions'
        body: dict = {'model': self._model_for(provider, model_role), 'messages': messages, 'temperature': temperature}
        if thinking == 'off':
        ...
        while True:
            attempt += 1
            try:
                resp = httpx.post(url, headers={'Authorization': f'Bearer {provider.api_key}', 'Content-Type': 'application/json'}, json=body, timeout=self.timeout)
            ...
            if resp.status_code == 429:
                ...
                if any((s in body for s in ('1113', '余额', 'insufficient', 'balance'))):
                    self.exhausted = True
                    raise QuotaExhausted(f'{provider.name} balance exhausted: {body[:120]}')
                rate_limited += 1
                if rate_limited > _RATE_LIMIT_MAX:
                    raise RuntimeError(f'{provider.name} HTTP 429 after {rate_limited} waits: {resp.text[:120]}')
    ...
    def _parse(self, provider: Provider, data: dict) -> ChatResult:
        ...
        self.budget.record(data.get('model', provider.model), usage.get('prompt_tokens', 0), usage.get('completion_tokens', 0))
        return ChatResult(content=msg.get('content') or '', tool_calls=calls, usage=usage, model=data.get('model', provider.model))
