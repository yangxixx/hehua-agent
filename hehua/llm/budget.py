...
class Budget:
    ...
    def __init__(self, soft_limit: int=150000000):
    ...
    def record(self, model: str, prompt_tokens: int, completion_tokens: int) -> None:
        with self._lock:
            m = self._by_model.setdefault(model, {'prompt': 0, 'completion': 0, 'calls': 0})
    ...
    def totals(self) -> dict:
        with self._lock:
            ...
            return {'prompt_tokens': prompt, 'completion_tokens': comp, 'total_tokens': prompt + comp, 'by_model': {k: dict(v) for k, v in self._by_model.items()}}
