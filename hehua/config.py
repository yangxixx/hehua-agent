"""Central configuration: env parsing + budget constants."""
...
class Config:
    ...
    reserve_min: int = 45
    level_budget_min: dict = field(default_factory=lambda: {
        "easy": 4, "medium": 8, "hard": 15, "default": 8,
    })
    # agent loop
    max_steps: int = 150
    tool_result_max: int = 8192      # head+tail cap per tool result (bytes)
    head_tail: int = 4096            # bash log head/tail size
    bash_timeout: int = 120
    bash_timeout_max: int = 300
    ctx_compact_ratio: float = 0.75  # compact when est. tokens > ratio * ctx
    ctx_limit: int = 60000           # conservative token budget per session
    token_soft_limit: int = 150_000_000
    # watchdog
    ...
    @property
    def mode(self) -> str:
        """local | hosted | mock — explicit env wins, else infer."""
...
def _load_dotenv(path: str = ".env") -> None:
    """Minimal .env loader (setdefault: real env wins)."""
    try:
...
def load_config() -> Config:
    return Config(
        ...
        qwen_compact_model=os.getenv("QWEN_COMPACT_MODEL", "qwen-turbo"),
        glm_api_key=os.getenv("GLM_API_KEY", ""),
        glm_model=os.getenv("GLM_MODEL", "glm-5.2"),
        kimi_api_key=os.getenv("KIMI_API_KEY", ""),
        kimi_model=os.getenv("KIMI_MODEL", "kimi-k3"),
        llm_provider=os.getenv("LLM_PROVIDER", "deepseek"),
        model_gateway=os.getenv("MODEL_GATEWAY", "0") == "1",
