...
def pick_playbook(description: str, code: str = "") -> str:
    ...
    # Unknown prefix (tomorrow's new-dimension codes, XBOW local practice):
    # HIGH-PRECISION keyword sniff for the AI / blockchain dimensions before
    # the web fallback. Keywords run ONLY for unknown prefixes — known prefixes
    # stay prefix-authoritative, so the run-6661 misrouting hazard (keywords
    # overriding prefixes) cannot recur. Keep these deliberately precise
    # ("智能合约"/"smart contract", never bare "contract"; no bare "agent"/"ai").
    ...
    if any(k in d for k in (
        ...
        return "playbook_blockchain.md"
    # Prefix-only routing (the run-6661 audit: keyword matching misrouted whole
    # classes — all 9 c-* got the web playbook). Unknown prefix (XBOW local
    # practice, or a pre-competition new-dimension code) falls back to web.
    return "playbook_web.md"
...
class Watchdog:
    ...
    def __init__(self, cfg: Config):
        ...
        self._stall_rounds = 0        # how many no-progress resets have fired
        # challenge context for the re-read-the-hint injection (#5)
        self.description = ""
        self.hint = ""
    ...
    def observe(self, tool_name: str, args: dict, progress: bool) -> list[str]:
        ...
        if progress:
            ...
            if self._steps_since_progress >= self.cfg.no_progress_steps:
                ...
                injections.append(self._stall_injection())
                self._steps_since_progress = 0
        return injections

    def _stall_injection(self) -> str:
        # Round 1: switch direction. Round 2+: the agent already switched once
        # and is STILL stuck — the #1 last-mile failure is locking onto the
        # first hypothesis and ignoring the challenge's own hint/title (xben-081
        # ground a downed DB while the "secret note" hint pointed elsewhere;
        # xben-088 found the race but never re-checked the admin gate logic).
        # Force a literal re-read of the hint and a hypothesis reset.
        if self._stall_rounds < 2:
            return ("[watchdog] No progress for 8 steps. Consult your playbook's "
                    "checklist and switch direction; record failed ideas as "
                    "notes(failure) and try an untested class of vulnerability.")
        msg = ("[watchdog] STILL stuck after a direction switch — you have likely "
               "locked onto your first hypothesis. STOP grinding it. Re-read the "
               "challenge hint/description below and ask: what does it LITERALLY "
               "point to that you have NOT tried? The 'obvious' blocker (a service "
               "that seems down, an auth that always errors, a field that 500s) is "
               "often the intended CLUE, not an obstacle.")
        if self.hint:
            msg += f"\nOFFICIAL HINT: {str(self.hint)[:300]}"
        if self.description:
            msg += f"\nDESCRIPTION: {str(self.description)[:400]}"
        msg += ("\nList 2 attack classes the hint/title suggests that you have NOT "
                "attempted, record them as notes(idea), then try the most likely "
                "one. Do NOT repeat anything already in your notes(failure).")
        return msg
...
def solve_challenge(ch: dict, addrs: list, cfg: Config, llm: LLMClient,
                    ...
                    system_name: str = "system.md") -> Outcome:
    ...
    # product PoC inventory: attach in-context for known-product classes
    # (killchain/exploit) so curated endpoints/CVEs are immediately to hand.
    if pb in ("playbook_killchain.md", "playbook_exploit.md"):
        try:
            system += "\n\n---\n" + (PROMPTS_DIR / "poc_inventory.md").read_text(encoding="utf-8")
        except OSError:
            pass
    ...
    watchdog.description = ch.get("description") or ""   # #5 re-read injection
    watchdog.hint = hint or ""
    ...
    # Cross-session dedup: tell the agent which flags were ALREADY submitted
    # correct on this challenge (prior attempts/peers; runner persists these to
    # submitted.json). It must hunt DIFFERENT flags / other stages — re-finding
    # an already-submitted flag wastes a round (task_uhS2QOLkhm7F ground out 30
    # "correct" submits, mostly dups of <=14 unique).
    try:
        import json as _sj
        _found = _sj.loads(
            (workdir / "submitted.json").read_text(encoding="utf-8")).get("correct", [])
        if _found:
            challenge_msg += ("\n\nFLAGS ALREADY SUBMITTED CORRECT on this challenge "
                              "(by a prior attempt/peer — do NOT re-find or re-submit "
                              "these; the leaderboard already credits them. Hunt "
                              "DIFFERENT flags: other stages, hosts, roles, params):\n- "
                              + "\n- ".join(str(f) for f in _found[:12]))
    ...
    while not tctx.finished and steps < cfg.max_steps:
        ...
        for call in res.tool_calls:
            ...
            if isinstance(result, str) and len(result) > 2048:
                result = _density_filter(result, llm, workdir)
...
def _density_filter(result: str, llm, workdir: Path) -> str:
    """#5: For large tool outputs (>2KB), extract key features via a cheap flash
    call before the result enters context. Raw output saved to a temp file for
    on-demand read_file. Prevents nuclei/HTML/large-scan noise from bloating
    context while preserving the evidence on disk."""
    low = result.lower()
    if "flag{" in low or "correct=true" in low or "submit" in low:
        return result
    import hashlib
    h = hashlib.md5(result.encode()[:1000]).hexdigest()[:8]
    rawfile = workdir / "out" / f"dense_{h}.txt"
    try:
        rawfile.parent.mkdir(parents=True, exist_ok=True)
        rawfile.write_text(result, encoding="utf-8", errors="replace")
    except OSError:
        return result
    try:
        res = llm.chat([
            {"role": "system",
             "content": "Extract 3-5 key security-relevant findings from this tool "
                        "output. Ultra-concise bullets. Include: framework/version "
                        "detected, exposed endpoints, error messages, sensitive data, "
                        "potential vulnerabilities, anything anomalous."},
            {"role": "user", "content": result[:8000]}
        ], temperature=0.0)
        summary = (res.content or "").strip()[:600]
        if not summary:
            return result
        return f"{summary}\n[raw output saved: {rawfile} — use read_file to inspect]"
    except Exception:  # noqa: BLE001
        return result
