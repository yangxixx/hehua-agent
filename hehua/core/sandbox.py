...
# Hard-coded guard: block raw bash networking loops that wedge runs.
# Does NOT block nmap/sqlmap/ffuf (legitimate tools with built-in rate limits).
_GUARD_PATTERNS = [
    (re.compile(r"/dev/(tcp|udp)/", re.IGNORECASE),
     "BLOCKED: /dev/tcp/ and /dev/udp/ raw bash networking is forbidden — it "
     "spawns runaway background jobs that wedge the run. Use the port_scan "
     "tool or nmap for port scanning."),
    (re.compile(r"seq\s+\d+\s+\d{3,}.*(nc |netcat|/dev/|bash -i)", re.IGNORECASE | re.DOTALL),
     "BLOCKED: mass-loop scan pattern detected (seq + network). Use port_scan "
     "tool, nmap, or scripts/scan.py instead — never a raw bash loop."),
]


def _bash_guard(command: str) -> str | None:
    """Return an error message if the command matches a blocked pattern, else None."""
    for pattern, msg in _GUARD_PATTERNS:
