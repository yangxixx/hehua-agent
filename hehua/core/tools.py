...
def build_tools(ctx: ToolContext, cfg) -> dict:
    ...
    def t_submit(flag, writeup=""):
        ...
        if flag in ctx.wrong_flags:
            return ("this exact flag was already proven WRONG on this "
                    "challenge — do not resubmit; change approach.")
        res = ctx.submit_fn(flag, str(writeup))
        ctx.last_submit = res
        if not res["correct"]:
            ctx.wrong_flags.add(flag)
            return ("incorrect — recorded as decoy/wrong; analyze WHY (decoy "
                    "cred? truncated value?) before trying variants.")
