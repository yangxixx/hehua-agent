"""CLI: python -m hehua {run|smoke|resume|report} [--mode local|hosted|mock]"""
...
def _build(cfg, mode: str):
    if mode == "mock":
        ...
        if llm is None:
            llm = LLMClient(providers_from_config(cfg),
                            Budget(cfg.token_soft_limit), gateway=cfg.model_gateway)
        return platform, llm
    from tsec_benchmark import TSecBenchmark
    platform = TSecBenchmark(base_url=cfg.benchmark_base_url,
                             token=cfg.benchmark_token)
...
def main(argv=None) -> int:
    ...
    if args.cmd == "smoke":
        ...
        os.environ["MOCK_LLM"] = "1"  # smoke is the scripted offline path
        cfg = load_config()
        ...
        # hard gate: the scripted LLM must solve BOTH mock targets — a
        # pipeline regression (tool loop, submit path, lifecycle) shows up
        # here before it can burn a real hosted run.
        unsolved = sorted(c for c, s in state.challenges.items()
                          if s.status != "solved")
        if unsolved:
            print(f"SMOKE FAIL: unsolved mock challenges {unsolved} "
                  f"(report: {report})")
            return 1
