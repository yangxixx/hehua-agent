...
def worker(base: str, q: 'Queue', out: list, lock: threading.Lock, timeout: float, baseline_len: dict):
    while True:
        path = q.get()
        if path is None:
            q.task_done()
...
def main(argv=None) -> int:
    ...
    ap.add_argument('-x', '--extensions', default='', help='comma list, e.g. php,txt,bak (appended to each word)')
    ...
    threads = [threading.Thread(target=worker, args=(args.base_url, q, out, lock, args.timeout, {}), daemon=True) for _ in range(n)]
    for t in threads:
        t.start()
    for c in candidates:
        q.put(c)
