...
def crawl(base: str, max_pages: int=60):
    ...
    seen, queue, found = (set(), [base], {'paths': set(), 'params': set(), 'forms': [], 'interesting': []})
    inter = re.compile('(flag|admin|backup|\\.git|api|debug|internal|upload)', re.I)
    while queue and len(seen) < max_pages:
        ...
        seen.add(url)
        try:
            r = urlopen(Request(url, headers={'User-Agent': 'hehua-crawl'}), timeout=10)
        ...
        for m in re.finditer('href=["\']([^"\']+)["\']|src=["\']([^"\']+)["\']', body):
        ...
        for k in parse_qs(urlparse(url).query):
            found['params'].add(k)
        for fm in re.finditer('<form[^>]*action=["\']?([^"\'>\\s]*)', body, re.I):
            found['forms'].append(urljoin(url, fm.group(1) or url))
        for nm in re.finditer('name=["\']([\\w-]+)["\']', body):
...
if __name__ == '__main__':
    print(json.dumps(crawl(sys.argv[1], int(sys.argv[2]) if len(sys.argv) > 2 else 60), indent=1))
