"""Fake Tsecbench platform with two REAL vulnerable targets (sqli + cmd inject).

Runs pure-stdlib HTTP servers on localhost so the full agent loop can be
exercised on Windows without docker/VPN.
"""
...
def _make_handler(db: sqlite3.Connection):
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            q = parse_qs(u.query)
            if u.path == "/":
                body = ('<html><a href="/user?id=1">user</a> '
                        '<a href="/ping?ip=127.0.0.1">ping</a></html>')
                self._send(200, body)
            elif u.path == "/user":
        ...
        def _user(self, idval: str):
            try:
                cur = db.execute(
                    f"SELECT name, password FROM users WHERE id = {idval}")
                rows = cur.fetchall()
                self._send(200, "users: " + "; ".join(
                    f"{n}/{p}" for n, p in rows))
            except Exception as e:  # noqa: BLE001 — leaks DB errors like real apps
...
class MockPlatform:
    def __init__(self):
        ...
        self.db.executemany(
            ...
            [(1, "alice", "alice123"), (2, "bob", "bob-pass"),
             (1337, "admin", FLAG_SQLI)])
        self.db.commit()
        ...
        self._challenges = [
            Ch("CH-SQLI", "Web app at target. Find the admin secret in the user "
                          "database (login as admin / read all rows)."),
            Ch("CH-CMD", "Ping diagnostic page at target. Achieve command "
                         "execution and read /etc/passwd."),
        ]
    ...
    def submit_flag(self, unique_code, flag):
        ...
        return _Submit(correct=correct, awarded=100 if correct else 0,
                       cumulative_score=100 * len(self._solved),
                       correct_flag_count=n, total_flag_count=1)
